...
namespace CollectionManagerSite.Controllers
{
    public class DetailsController : ApiController
    {
        ...
        public string GetCollectionStatus(int id, string type)
        {
            ...
            if (kiosk != null)
            {
                var indexes = CalculateGottenEmblemIndexes(type, WebClient.MembershipIds, consoleChoice);
                foreach (var index in indexes)
                {
                    var item = kiosk.itemList[index];
        ...
        private List<int> CalculateGottenEmblemIndexes(string type, string[] membershipIds, int consoleChoice)
        {
            ...
            var gottenEmblemIndexes = kioskCollection.Response.profileKiosks.data.kioskItems[622587395].Where(ki => ki.canAcquire).Select(ki => ki.index).ToList();

            return gottenEmblemIndexes;
        }
